...
const Ae = ["innerHTML"], Se = /* @__PURE__ */ ie({
  ...
  setup(B, { expose: J }) {
    ...
    function K() {
      ...
      H.position.set(5, 10, 5), c.add(H), console.log("光源设置完成"), S = new me(), c.add(S), console.log("热力图组创建完成"), F = new de(), M = new ve(), $(), console.log("Three.js 初始化完成");
    }
    function Q() {
    ...
    function $() {
      ...
      for (let n = 0; n < f; n++) {
        ...
        if (Array.isArray(b))
          for (let h = 0; h < g; h++) {
            const p = typeof b[h] == "number" ? b[h] : 0, E = (p - r) / (t - r), X = 0.1 + E * o.maxHeight;
            H[m] = E;
